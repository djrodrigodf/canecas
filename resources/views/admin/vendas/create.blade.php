...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.venda.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.vendas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="cliente">{{ trans('cruds.venda.fields.cliente') }}</label>
                <input class="form-control {{ $errors->has('cliente') ? 'is-invalid' : '' }}" type="text" name="cliente" id="cliente" value="{{ old('cliente', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('produto') ? 'is-invalid' : '' }}" name="produto" id="produto" required>
                    ...
                    @foreach(App\Venda::PRODUTO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('produto', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('frase') ? 'is-invalid' : '' }}" name="frase" id="frase">{{ old('frase') }}</textarea>
                @if($errors->has('frase'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pago') ? 'is-invalid' : '' }}" name="pago" id="pago" required>
                    ...
                    @foreach(App\Venda::PAGO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('pago', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('pago'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('entregue') ? 'is-invalid' : '' }}" name="entregue" id="entregue">
                    ...
                    @foreach(App\Venda::ENTREGUE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('entregue', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
